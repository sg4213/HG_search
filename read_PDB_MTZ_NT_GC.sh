...
if [ -f "$FILE" ]; then
  echo "omit.updated.pdb $mtz_file omit.ligands.cif $EXTRA_FLAGS main.number_of_macro_cycles=5" >> phenix.refine.txt
  phenix.refine omit.updated.pdb $mtz_file omit.ligands.cif strategy=individual_sites+individual_adp+occupancies main.number_of_macro_cycles=5
  ...
  phenix.refine omit.pdb $mtz_file strategy=individual_sites+individual_adp+occupancies main.number_of_macro_cycles=5
...
rm check_occupancy.py
find . -maxdepth 1 -type f \
  ! -name "*.log" \
  ! -name "${pdb_id}_final_refine_001.mtz" \
  ! -name "${pdb_id}_final_refine_001.pdb" \
  -delete
...
cmd.png(f\"{pdb_id}_{chain_1}_{nt_number_1}_{chain_2}_{nt_number_2}_WC_map_water.png\", width=1200, height=900, ray=1, dpi=600)
cmd.quit() "
...
find . -maxdepth 1 -type f \
  ! -name "*.log" \
  ! -name "${pdb_id}_final_flipped_refine_001_refine_001.mtz" \
  ! -name "${pdb_id}_final_flipped_refine_001_refine_001.pdb" \
  -delete
...
# Render the scene as a PNG image and then exit
cmd.png(f\"{pdb_id}_{chain_1}_{nt_number_1}_{chain_2}_{nt_number_2}_HG_map_water.png\", width=1200, height=900, ray=1, dpi=600)
cmd.quit() "
